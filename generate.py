...
### Read WMT events schema
with open('_data/_wmt_events_schema.json', 'r', encoding='utf-8') as f:
  WMT_EVENTS_SCHEMA = json.load(f)

### Read external Calls For Papers
with open ('_data/calls_for_papers.json', 'r', encoding='utf-8') as stream:
  CALLS_FOR_PAPERS = json.load(stream)

### Read external Calls For Papers schema
...
for event in EVENTS:
  ...
  if event.get('location', None):
    event_attendance_mode = event['location'].get('eventAttendanceMode', None)
    location_name = event['location'].get('name', None)
    location_url = event['location'].get('url', None)
    location_type = event['location'].get('type', None)
...
for entry in CALLS_FOR_PAPERS:
  ...
  datetime.strptime(entry['callsForPapersDeadline'], "%Y-%m-%d").date()
