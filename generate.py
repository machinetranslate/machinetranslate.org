...
for api in APIS:
  ...
  if api.get('voice_translation', False):
    translation_types.append('Voice Translation')
...
''')
  ...
  ### Write input types
  keys_to_extract = ['autogenerated', 'autogenerated_from', 'nav_order', 'redirect_from', 'title', 'id', 'translation_types', 'active']
  filtered_frontmatter = {k: frontmatter.get(k if k in frontmatter else k.lower()) for k in keys_to_extract}

  if api.get('voice_translation', False):
    filtered_frontmatter['grand_parent'] = 'Input types'
    filtered_frontmatter['parent'] = 'Voice'
    filtered_frontmatter['redirect_to'] = api_id
    filepath = f'input-types/voice/{ api_id }-type.md'
    content = read_content(filepath)
    with open(filepath, 'w', encoding='utf-8') as f:
      f.write(f'''\
---
{ yaml.dump(filtered_frontmatter, sort_keys=False, allow_unicode=True) }
---
{ content }
''')
