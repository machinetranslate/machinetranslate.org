...
### Read routers
with open('_data/routers.json', 'r', encoding='utf-8') as stream:
...
for api in TRANSLATION_APIS:
  ...
  frontmatter = {
    'autogenerated': True,
    'autogenerated_from': 'trnaslation_apis.json',
    'nav_order': 1000 - (len(supported_languages) + len(integrations)),
    'layout': 'translation_api',
    'title': name,
    'description': desc,
    'id': api_id,
    'parent': 'Trnaslation APIs',
    'urls': urls,
...
### Write languages
API_SUPPORTED_LANGUAGE_BASE_CODES = supported_language_base_codes(TRANSLATION_APIS)
...
for tms in INTEGRATIONS:
    ...
    for integration in tms['translation_api_integrations']:
      try:
        integration_data = {}
    ...
    frontmatter = {
      ...
      'layout': 'integration',
      'nav_order': 1000 - (len(translation_api_integrations) + len(tms_qe_api_integrations)),
      'title':  tms_name,
      'description': desc,
      'id': tms_id,
      'parent': 'Integrations',
      ...
      'tms_url': tms_url,
      'translation_api_integrations': translation_api_integrations,
...
for r in ROUTERS:
    ...
    for r in r['supported_apis']:
      try:
        ...
        if isinstance(r, dict):
          r_slug = list(r.keys())[0]
          r_data['slug'] = r_slug
          for k, v in r[r_slug].items():
            r_data[k] = v
        else:
          r_slug = r
          r_data['slug'] = r_slug
    ...
    frontmatter = {
      ...
      'layout': 'router',
      'nav_order': 1000 - len(r_supported_apis),
      'title': r_name,
      'description': desc,
      'id': r_id,
      'parent': 'Routers',
...
for estimation in QUALITY_ESTIMATION:
  ...
  if only_compatible_mt_api_id:
    api_name = next((api['name'] for api in TRANSLATION_APIS if only_compatible_mt_api_id and \
